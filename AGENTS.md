...
- 每个小步完成时更新 `docs/current-state.md`，只记录该小步实际完成内容、两轮检查证据、分支/提交状态和明确未完成项。
...
4. 第一次检查做静态与一致性验证，例如配置/SQL 参数/调用次数/链接/`git diff --check`。
5. 第二次检查做可执行验证；按风险运行专属测试、真实 fixture PostgreSQL 集成测试、完整 pytest、bundle/PDF 视觉检查和 `pip check`。
6. 把真实结果写入 `docs/current-state.md`，不得复制上一步的过期测试数量。
7. 只暂存本步文件，检查 staged diff，创建一笔语义清楚的本地 commit 并 push 到当前功能分支。
8. 阶段完成后走 Draft PR → CI → 用户确认 → merge commit → main CI → 下一阶段新分支。
