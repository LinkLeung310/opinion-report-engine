...
1. 完整读取本文件。
2. 完整读取 `docs/requirements-traceability.md`；若用户在当前任务中重新提供仓库外任务书，也完整读取该附件。
...
- 面试方原始任务书不保存在仓库；用户在当前任务中重新提供时只作外部输入读取，不复制进版本控制。
- `docs/final-framework.zh-CN.md`：以用户体验为核心的产品框架，说明用户如何选章节、生成、等待、阅读和处理局部失败。
- `docs/architecture.md`：Python 引擎的模块边界、执行流、依赖注入、失败语义和发布方式。
- `docs/02-report-spec.md`：逐章技术合同；每章的输入、固定 SQL、Python 事实、证据、图表、一次 narrator 和 no-data 行为在实现前写清楚。
- `docs/design-decisions.md`：任务书没有规定、但项目已经做出的自主决定。不得把这些决定伪装成面试方要求。
- `docs/requirements-traceability.md`：仓库内持久化的任务书条款摘要，以及到实现与验证证据的映射。
- `docs/current-state.md`：当前真实进度和最新验证，不定义新需求，也不把“计划”写成“已完成”。

`AGENTS.md` 只保存长期治理规则和上述导航，不复制 19 章的全部细节。恢复上下文时按这里的职责读取唯一来源，避免多份文档长期漂移。
...
- 一个功能分支只承载一个清晰阶段或纵向切片；当前阶段未完成时不混入下一阶段或 RAG。
- 一笔 commit 只表达一个可审查意图；优先按“规格决定 → SQL/事实 → runner/图表/产物 → 文档/验收”拆分，提交信息用 `feat:`、`fix:`、`test:`、`docs:`、`ci:` 等前缀说明结果。
- 小步提交首先是本地 Git commit；只有 push 到 GitHub 后评审者才能看到。
- 每个已验证的小步都 push 到当前 GitHub 功能分支。push 不是合并：评审者通过分支和 PR 查看历史，`main` 仍保持绿色。
- 推送前运行与风险相称的测试；涉及 SQL 时必须跑真实 fixture PostgreSQL 集成测试，涉及 LLM 时必须跑 stub 测试。
- 推送功能分支后开 Draft PR。PR 必须说明任务书映射、自主决定、当前未完成项和测试证据。
- 一个阶段的 Draft PR 在 CI 通过、无已知阻断项并取得用户确认后再合并。默认使用 merge commit 保留小步历史，不擅自 squash。
- 合并后先同步本地 `main` 并确认 main CI 绿色，再从这个最新绿色提交创建下一阶段 `codex/<scope>` 分支；禁止从旧功能分支继续串联开发。
...
- 每个小步运行一次与风险相称的合并检查；阶段收口、PR ready 和合并前再运行 `python -m pip check` 与完整 `pytest`。把实际结果写进 PR，不复制过期数字，也不为形式重复同一范围。

“文件存在”“进程启动”或“测试曾经通过”都不是当前完成证据。必须检查实际产物、健康状态或最新 CI。

## 8. 状态维护和 context recovery

- 每个小步开始前都重新完整读取本文件；聊天仍在同一任务中也不能跳过。
- 每个小步完成时更新 `docs/current-state.md`，只记录该小步实际完成内容、一次合并检查证据、分支/提交状态和明确未完成项。
- 每个合并 PR 后再次更新 `docs/current-state.md`：合并提交、main CI、已完成项、已知差异和下一推荐目标。
...
1. 读取 `AGENTS.md`、`requirements-traceability`、`current-state` 和本步相关设计；若当前任务附有外部任务书则一并读取；检查分支与脏工作区。
2. 先声明本步范围、任务书映射、自主决定和明确不做的内容。
3. 只实现一个可审查意图，不顺手扩大范围。
4. 做一次与风险相称的合并检查，同时覆盖必要的静态一致性和可执行行为；涉及 SQL 必须使用真实 fixture PostgreSQL，涉及 LLM/RAG 必须使用 stub/fake，影响 bundle/PDF 时检查真实产物。小步优先跑专属范围，阶段收口再跑完整 pytest、`pip check` 和必要视觉检查，避免重复运行已经有充分证据的相同范围。
5. 把真实结果写入 `docs/current-state.md`，不得复制上一步的过期测试数量。
6. 只暂存本步文件，检查 staged diff，创建一笔语义清楚的本地 commit 并 push 到当前功能分支。
7. 阶段完成后走 Draft PR → CI → 用户确认 → merge commit → main CI → 下一阶段新分支。

开发与 CI 始终使用可注入 stub/fake。真实 OpenAI-compatible API 不参与日常开发或验收，只在全部本地功能与自动化测试完成后，经凭据门控执行最终冒烟测试；密钥不得写入仓库、日志、PR 或聊天正文。
