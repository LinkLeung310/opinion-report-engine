...
| R-15 | M3：提交、状态、下载；两个任务互不干扰；重启后成品可下载 | D-41 HTTP task contract + D-42 有界进程内 manager；每任务独立数据库连接/引擎，原子磁盘状态和 ZIP 下载 | 202/状态/错误/幂等/队列测试；真实 fixture 两任务隔离；重启后 PDF/ZIP 下载与中断任务收口 | 已决策 |
