...
| R-15 | M3：提交、状态、下载；两个任务互不干扰；重启后成品可下载 | D-41 HTTP task contract + D-42 有界进程内 manager；每任务独立数据库连接/引擎，原子磁盘状态和 ZIP 下载 | 已有状态机、容量、资源上下文、幂等重放、ZIP 与重启收口单测；仍需 HTTP/OpenAPI 及真实 fixture 双任务端到端验证 | 实现中 |
