...
最后核对日期：2026-07-17
最后实现基线：`codex/m3-api-local`（M3 本地功能分支；`main` 仍为 `5909005`）
...
- M3 已完成 HTTP/队列/恢复规格、持久化状态机、有界 JobManager、原子 ZIP、FastAPI HTTP/OpenAPI 与真实运行时资源工厂；真实 fixture 双任务 API 验收仍未完成，因此 M3 不能宣称完成。
- n8n 仍未对已实现的 M3 API 做真实端到端验收，必须保持 inactive，不得声称集成完成。
...
- 事务隔离基线之后的英文 19 章矩阵、代表性任意组合、OpenAI-compatible narrator 与 CatalogPublisher 已在各自独立本地功能分支完成并合入 `codex/local-integration`；下一阶段从该本地基线实现 M3，不混入 RAG 或提前激活 n8n。
...
- API 规格遵循当前 FastAPI 官方建议：用 lifespan 管理 manager 生命周期，以 202 表达响应后处理，并使用文件响应下载；实际依赖和实现尚未添加。本仓库的决定唯一来源仍是 `docs/design-decisions.md`，没有另建技能建议的重复 `.gsd/DECISIONS.md`。
...
- 本 HTTP 小步没有修改固定配置、章节 SQL/事实/图表/PDF/bundle、RAG 或 n8n，也未连接真实模型、执行 push、PR 或远程写操作。M3 仍需真实 fixture PostgreSQL + StubNarrator 的两个并发 API 任务、重启后 HTTP 下载和完整阶段回归后才能收口。
