...
最后核对日期：2026-07-17
最后实现基线：`codex/m2-english-local`（M2 本地验收阶段；`main` 仍为 `5909005`）
...
- M2 的 19 个章节纵向切片、完整英文 stub 矩阵、三类专属输入和代表性任意重排组合均已完成本地验收；真实 OpenAI-compatible narrator 仍是下一独立阶段。
...
- 事务隔离基线之后的英文 19 章矩阵与代表性任意组合已在独立本地功能分支完成；下一阶段实现真实 OpenAI-compatible narrator，不混入 RAG、M3 或 n8n。
...
- M2 的确定性/stub 验收已完成。真实 OpenAI-compatible narrator、RAG、M3、CatalogPublisher 和 n8n 仍未开始；不得把本阶段的 stub 证据描述成真实模型冒烟。
...
- CLI 尚未切换到真实 narrator；本小步未调用真实模型、未新增凭据，也未实现 RAG、CatalogPublisher、M3 或 n8n。
