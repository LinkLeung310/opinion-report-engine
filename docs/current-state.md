...
context recovery、verdict、trend、platforms 和 severity slice 已经合并。当前分支 `codex/m1-risk-section` 从绿色 `main@01b0cef` 创建；`risk` 规格与 D-22 已定义，尚未开始该章节实现。用户要求暂不开始 RAG，因此先推进结构化 `risk`，不进入预留的证据密集 `viewpoints`，也不新增 embedding、vector store、retriever 或 n8n 节点。
...
- `risk` 规格和 D-22 已定义；规格检查通过后，下一小步实现固定 SQL、typed snapshot、FactSet 和真实 fixture 集成测试，不接图表、narrator 或真实 API。
