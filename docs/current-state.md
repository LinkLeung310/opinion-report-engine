...
- 事务隔离基线之后的英文 19 章矩阵、代表性任意组合和 OpenAI-compatible narrator 已在各自独立本地功能分支完成；下一阶段实现 `CatalogPublisher`，不混入 RAG、M3 或 n8n。
...
- 本规格小步不实现代码，不修改公共输入、章节、SQL、图表、PDF、RAG、M3 或 n8n；下一小步才添加 catalog adapter、故障/并发测试及 application service 接线。
