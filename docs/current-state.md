...
context recovery、verdict、trend、platforms 和 severity slice 已经合并。当前分支 `codex/m1-risk-section` 从绿色 `main@01b0cef` 创建；`risk` 规格及 SQL/计算层已经实现，图表、runner、运行时接线与 CLI 产物尚未实现。用户要求暂不开始 RAG，因此先推进结构化 `risk`，不进入预留的证据密集 `viewpoints`，也不新增 embedding、vector store、retriever 或 n8n 节点。
...
- `risk` 规格、固定 SQL、typed snapshot、FactSet 和真实 fixture 集成测试已完成；下一小步实现五条横向图、fault-isolated runner、stub narrator、运行时接线与 CLI 产物，不接真实 API。
