...
context recovery、verdict、trend、platforms 和 severity slice 已经合并。当前分支 `codex/m1-risk-section` 从绿色 `main@01b0cef` 创建，准备定义 M1 `risk` 规格；尚未开始该章节实现。用户要求暂不开始 RAG，因此先推进结构化 `risk`，不进入预留的证据密集 `viewpoints`，也不新增 embedding、vector store、retriever 或 n8n 节点。
...
- 下一小步先扩展 `docs/02-report-spec.md` 和设计决定，定义 `risk` 的输入、固定 SQL、Python 派生事实、证据边界、图表、一次 narrator 和 no-data/failed 行为；规格检查通过前不写实现。
