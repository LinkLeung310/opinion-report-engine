...
最后核对日期：2026-07-17
最后实现基线：`codex/local-integration@ceceb53`（本地事务隔离基线；`main` 仍为 `5909005`）
...
- 事务隔离小步已完成 PostgreSQL 章节查询故障隔离、真实数据库回归、D-37/追踪矩阵和本地基线复验；下一阶段在独立本地功能分支完成英文 19 章矩阵与任意组合，不混入 RAG、M3 或 n8n。
...
- 报告外壳聚焦检查覆盖 assembler、application service 和 ReportLab renderer，共 15 项通过；Python 静态编译与 `git diff --check` 通过。该证据尚未覆盖章节自身事实枚举、no-data/failed 分支或 15 张图表，因此 M2 仍未完成。
...
- M2 仍未完成：尚需把英文全 19 章配置固化为仓库示例，并用自动化测试覆盖任意重排组合、三类专属输入、局部失败/no-data、完整 bundle 与 PDF 验收。
- RAG、M3、CatalogPublisher、n8n 和真实模型 API 均未开始。
