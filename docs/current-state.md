...
最后核对日期：2026-07-17
最后实现基线：`codex/local-integration@555b42c`（M3 已本地合并；`main` 仍为 `5909005`）
...
- n8n 仍未对已实现的 M3 API 做真实端到端验收，必须保持 inactive，不得声称集成完成；n8n 是项目演示层，不是任务书 M3 完成条件。
...
- 事务隔离基线之后的英文 19 章矩阵、代表性任意组合、OpenAI-compatible narrator、CatalogPublisher 与 M3 均已在各自本地功能分支完成并合入 `codex/local-integration`；M3 本地 merge commit 为 `555b42c`，`main@5909005` 未移动。
...
- RAG 继续延期；n8n 保持 Draft/inactive，下一独立小步只验证其调用已完成 M3 API 的演示流程。
