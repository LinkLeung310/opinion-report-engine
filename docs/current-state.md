...
- 跨语言审计合同与 D-37 明确：`language: "en"` 必须覆盖报告标题/范围/方法、计算标签与单位、图表、no-data/failed 文案和 PDF metadata；真实标题、摘要、平台/专名、精确关键词、Evidence ID 与用户上下文保持原文，不能为了“全英文”篡改来源。
- 新增 `examples/report-config.m2-all.en.json`，一次启用全部 19 章并提供 `responseDate`、`comparisonTag` 和 `notes`。真实 fixture PostgreSQL + stub CLI 审计得到 19 requested/19 complete/0 failed、15 张声明图表且磁盘恰好 15 个 PNG，证明逐章正常路径已接通。
- 同一审计也证明 M2 英文尚未完成：meta 标题仍为 `Bilibili dislike-control algorithm adjustment舆情分析报告`，Markdown 总标题、监测范围和方法说明固定中文；部分百分点评价/阶段标签仍是中文格式值；全部 15 个 ChartBuilder 都不接收 `language` 且图题/轴/图例/注释大量硬编码中文；12 个旧 runner 的 failed 路径固定中文。以上是必须修复的真实差异，不能用“19 章 complete”静默掩盖。
- 审计小步单次检查以真实全英文 19 章 CLI bundle 为可执行证据，并用静态扫描定位 assembler、chart builder 接口、计算显示值和 runner failure 的语言泄漏；本小步只新增验收配置与合同/决定/状态文档，不修改运行行为、fixtures、RAG 或 n8n。
