...
| 2.1 输入契约 | Exact config fields, 19-section order, unknown type fallback | Final framework §2; trace R-01/R-02 | PASS; exact IDs await supplied spec |
| 2.2 输出契约 | `out/{id}`, md/pdf/charts/meta and fixed metadata fields | Final framework §3; trace R-03/R-04 | PASS; frontend failure field awaits type |
...
| 核心要求 4 | Section failure isolation, visible missing mark, meta failure, retry/backoff | Final framework §§3/8/9; trace R-06/R-09 | PASS with retry conflict documented |
| 核心要求 5 | `.env` for PG and OpenAI-compatible LLM; no secrets | Final framework §§8/11; trace R-10 | PASS |
| 核心要求 6 | Cross-platform A4 PDF, CJK fonts, gold CSS | Final framework §10; trace R-11 | PASS; CSS details await asset |
...
| 五、开发环境 | Fixture Docker, env, sample SQL, gold report, 30-minute setup | Final framework §§14/17; trace R-16 | PASS at framework level; assets absent |
| M1 | CLI, Chinese 7/11 defaults, exact numbers, PDF quality, failure isolation | Final framework §§13–15; trace R-13 | PASS at framework level; examples/fixtures absent |
| M2 | 19 sections, three input sections, English, arbitrary combinations | Final framework §§2/15/17; trace R-14 | PASS at framework level; spec absent |
...
- The two textual contradictions (LLM retry vs call cap; failure metadata vs shown
  frontend schema) have explicit temporary decisions and remain visible.
- Missing repository assets are not replaced by invented schemas, SQL, section IDs, or
  CSS. They are listed in final framework §17 and must be inspected before implementation.
